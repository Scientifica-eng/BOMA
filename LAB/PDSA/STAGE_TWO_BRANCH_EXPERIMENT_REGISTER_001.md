...
**Status:** `ST2-EXP-001..004 + ST2-EXP-011 CLOSED / PASS / ST2-EXP-004 ARCHITECTURE INTEGRATION PENDING / ACTIVE EXPERIMENT = NONE`  
...
The owner authorized completion of the current `ST2-EXP-004` lifecycle and the
separate architecture integration of its durable lesson, followed by a stop
...
| --- | --- | --- | --- | --- |
| `ST2-EXP-001` | `DEPENDENCY_EDGE / BOMA-C-R-DEP-001` | `PDSA-ST2-EXP-001_NARROW_R_INTERFACE.md` | `CLOSED / PASS` | none; sixteen-property R→C production lesson integrated |
| `ST2-EXP-002` | `DECISION_POINT / C-DP-001` | `PDSA-ST2-EXP-002_INDEPENDENT_QUOTIENT_ROUTE.md` | `CLOSED / PASS` | none; Route P remains selected, Route Q remains research-only |
| `ST2-EXP-003` | `DECISION_POINT / R-DP-001` | `PDSA-ST2-EXP-003_INDEPENDENT_CAUCHY_REAL_ROUTE.md` | `CLOSED / PASS` | none; Dedekind remains selected, Cauchy evidence remains research-only |
...
| `ST2-EXP-004` | `DECISION_POINT / R-DP-003` | `PDSA-ST2-EXP-004_R_TOTAL_ORDER_LOGICAL_REGIME.md` | `CLOSED / PASS / ARCHITECTURE INTEGRATION PENDING` | none; Stage-I localized classical totality provider retained |
...
15-property interface; current `C-CL-FIELD-001` and `C-CL-INTEGRATION-001`
proof closures do not. `C-CL-COMPARE-001` survives and remains governed by the
narrow ST2-EXP-011 comparison interface.
...
Failure records `ST2-EXP-004_FAILURE_001` through `012` remain preserved as
execution evidence.
...
Closing an experiment does not promote its research branch. The following
remain unchanged unless a separate accepted architectural decision explicitly
changes them:

```text
R-DP-001 SELECTS R-ROUTE-D
R-DP-003 selected localized classical comparability for Stage I
...
## Required next act and stop condition

The only remaining authorized work is:

1. exact-head lifecycle-closed verification of `ST2-EXP-004`;
2. research/lifecycle merge;
3. separate maintenance Learning-to-Construction integration;
4. exact-head verification and integration merge;
5. stop with `ACTIVE EXPERIMENT = NONE`.
