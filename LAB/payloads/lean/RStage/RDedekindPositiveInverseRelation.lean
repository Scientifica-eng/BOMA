/- Representative-independent positive inverse relation for selected Dedekind reals.
This avoids defining a global reciprocal function by Choice. -/
namespace BOMA.R.DedekindPositiveInverseRelation001
...
open BOMA.R.DedekindPositiveRepresentative001
open BOMA.R.DedekindPositiveInverseProduct001

/-- Positive inverse relation: y is represented by the direct reciprocal of a
positive representative of x. The relation stores witnesses but chooses no
global representative or reciprocal function. -/
def RPositiveInvRel (x y : RBOMA) : Prop :=
  ∃ A : LowerCut, ∃ hApos : CutHasPositive A,
    x = rmk A ∧
    y = rmk (cutRecipPos A hApos)

/-- The direct positive reciprocal always contains the principal zero cut. -/
...
Witness extraction is delegated to the separately audited representative theorem. -/
...
/-- Positive inverse witnesses are representative-independent and unique at the
quotient level. -/
...
end BOMA.R.DedekindPositiveInverseRelation001
