/- Corrected positive representative witnesses for nonzero nonnegative selected reals.
Revision 002 preserves revision 001 as a proof-engineering learning artifact. -/
namespace BOMA.R.DedekindPositiveRepresentative002
...
theorem r_nonneg_nonzero_positive_rep
  ...
  have hneCut : ¬ CutEquiv A (principalCut qZero) := by
    intro hEq
    apply hAne
    change rmk A = rmk (principalCut qZero)
    exact rmk_sound hEq
  have hApos : CutHasPositive A :=
    cut_has_positive_of_nonneg_ne_zero h0Acut hneCut
  exact ⟨A, hApos, rfl⟩

end BOMA.R.DedekindPositiveRepresentative002
