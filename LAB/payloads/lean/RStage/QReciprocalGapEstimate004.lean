/- Rational reciprocal-gap estimate revision 004.
Preserves 001-003 as PDSA proof-engineering history. -/
namespace BOMA.R.QReciprocalGap004
...
open BOMA.R.QPositiveMultiplicativeApprox001
 ...
 theorem q_add_one_minus (q : QBOMA) :
  ...
  calc
    qAdd q (qAdd qOne (qNeg q)) = qAdd qOne (qAdd q (qNeg q)) := by
      calc
        qAdd q (qAdd qOne (qNeg q)) = qAdd (qAdd q qOne) (qNeg q) :=
 ...
 theorem q_add_gap (a r : QBOMA) :
  ...
  calc
    qAdd a (qAdd r (qNeg a)) = qAdd r (qAdd a (qNeg a)) := by
      calc
        qAdd a (qAdd r (qNeg a)) = qAdd (qAdd a r) (qNeg a) :=
 ...
 theorem q_lt_mul_inverse_of_small_gap
    ...
    (hgapSmall : qLT (qAdd r (qNeg a)) (qMul (qAdd qOne (qNeg q)) a0))
  ...
  have hqg : qLT (qMul q g) g := by
    ...
    exact ht
  have hqa_qag :
      qLT (qAdd (qMul q a) (qMul q g)) (qAdd (qMul q a) g) := by
    have ht := qlt_add_right hqg (qMul q a)
    rw [qAdd_comm (qMul q g) (qMul q a), qAdd_comm g (qMul q a)] at ht
    exact ht
  have hqag_final :
      qLT (qAdd (qMul q a) g) (qAdd (qMul q a) (qMul omq a)) := by
    have ht := qlt_add_right hgapToA (qMul q a)
    rw [qAdd_comm g (qMul q a), qAdd_comm (qMul omq a) (qMul q a)] at ht
    exact ht
  have hsumA : qAdd (qMul q a) (qMul omq a) = a := by
    calc
      qAdd (qMul q a) (qMul omq a) = qMul (qAdd q omq) a :=
        (qMul_add_left q omq a).symm
  ...
  have hmid :
    ...
    qlt_trans hqa_qag hqag_final
  have hqr : qLT (qMul q r) a := by
    have ht := hmid
    rw [← hqexpand, hsumA] at ht
    exact ht
