/- Rational gap estimate used by the positive real inverse-product theorem. -/
namespace BOMA.R.QReciprocalGap001
...
/-- Local strict/non-strict transitivity in the other orientation. -/
...
/-- q + (1-q) = 1 in the accepted rational additive group. -/
...
This theorem is purely rational: no LowerCut, completeness, or real carrier occurs here. -/
theorem q_lt_mul_inverse_of_small_gap
  ...
  have hgapPos' : qLT qZero g := by
    exact hgapPos
  have hgapSmall' : qLT g (qMul omq a0) := by
    exact hgapSmall
