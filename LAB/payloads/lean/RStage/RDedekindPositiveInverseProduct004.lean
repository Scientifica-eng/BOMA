/- Positive inverse-product revision 004.
Preserves earlier revisions; fixes only proof-witness scope after destructuring. -/
namespace BOMA.R.DedekindPositiveInverseProduct004
...
open BOMA.R.QReciprocalGap004
 ...
 theorem principal_one_mem_mul_recip_of_nonneg
  ...
  have hAposOrig : CutHasPositive A := hApos
  ...
  have hProofEq : CutEquiv (cutRecipPos A hLocalPos) (cutRecipPos A hAposOrig) :=
    cutRecipPos_proof_independent A hLocalPos hAposOrig
  have hbRecip : (cutRecipPos A hAposOrig).lower b := (hProofEq b).mp hbLocal
  exact Or.inr ⟨a', b, Or.inl ha'A, Or.inl hbRecip, h0a', h0b, hqab⟩

 theorem cutMul_recipPos_equiv_one
    (A : LowerCut)
    (hApos : CutHasPositive A) :
    CutEquiv (cutMulNonnegEnvelope A (cutRecipPos A hApos)) (principalCut qOne) := by
  intro q
  constructor
  · intro hq
    ...
    · rcases hqProd with ⟨a, b, haPos, hbPos, h0a, h0b, hqab⟩
      ...
      · have har : qLT a r := cut_inside_lt_outside haA hrOut
        have h0rinv : qLT qZero rinv := qlt_zero_inverse h0r hrinv
        have hab_arinv : qLT (qMul a b) (qMul a rinv) := qmul_lt_left_positive hbInv h0a
        have har_rinv : qLT (qMul a rinv) (qMul r rinv) := qmul_lt_right_positive har h0rinv
...
end BOMA.R.DedekindPositiveInverseProduct004
