...
/-- The selected canonical carrier is exactly the verified Route P record. -/
abbrev CBOMA : Type := BOMA.C.RoutePairProbe001.PairCandidate

/-- The selected identity is Lean record equality, equivalent to coordinate equality. -/
theorem c_ext {z w : CBOMA}
    (hre : z.re = w.re) (him : z.im = w.im) : z = w :=
  BOMA.C.RoutePairProbe001.pair_ext hre him

def cZero : CBOMA := BOMA.C.RoutePairProbe001.zero
def cOne : CBOMA := BOMA.C.RoutePairProbe001.one
def cOfR (a : RBOMA) : CBOMA := BOMA.C.RoutePairProbe001.ofR a
def cI : CBOMA := BOMA.C.RoutePairProbe001.imag
def cNeg (z : CBOMA) : CBOMA := BOMA.C.RoutePairProbe001.neg z
def cAdd (z w : CBOMA) : CBOMA := BOMA.C.RoutePairProbe001.add z w
def cMul (z w : CBOMA) : CBOMA := BOMA.C.RoutePairProbe001.mul z w
...
theorem c_zero_ne_one (C : RStageIntegrationCertificate) :
    cZero ≠ cOne := by
  intro h
...
theorem c_ofR_injective {a b : RBOMA} (h : cOfR a = cOfR b) : a = b :=
  BOMA.C.RoutePairProbe001.ofR_injective h

theorem c_add_comm (C : RStageIntegrationCertificate) (z w : CBOMA) :
...
theorem c_add_zero_left (C : RStageIntegrationCertificate) (z : CBOMA) :
    cAdd cZero z = z := by
  apply c_ext
  · exact C.addZeroLeft z.re
  · exact C.addZeroLeft z.im

theorem c_add_zero_right (C : RStageIntegrationCertificate) (z : CBOMA) :
    cAdd z cZero = z := by
  rw [c_add_comm C z cZero, c_add_zero_left C z]

theorem c_add_neg_right (C : RStageIntegrationCertificate) (z : CBOMA) :
    cAdd z (cNeg z) = cZero := by
  apply c_ext
  · exact C.addInverseRight z.re
  · exact C.addInverseRight z.im

theorem c_add_neg_left (C : RStageIntegrationCertificate) (z : CBOMA) :
    cAdd (cNeg z) z = cZero := by
  rw [c_add_comm C (cNeg z) z, c_add_neg_right C z]

theorem c_neg_involutive (C : RStageIntegrationCertificate) (z : CBOMA) :
    cNeg (cNeg z) = z := by
  apply c_ext
  · exact neg_involutive_from_interface C z.re
  · exact neg_involutive_from_interface C z.im

theorem c_mul_comm (C : RStageIntegrationCertificate) (z w : CBOMA) :
...
theorem c_mul_one_left (C : RStageIntegrationCertificate) (z : CBOMA) :
    cMul cOne z = z := by
  apply c_ext
  · change
    ...
    rw [C.mulOneLeft z.re, mul_zero_left_from_interface C z.im,
      ...
      BOMA.C.RoutePairProbe001.add_zero_right_from_interface C z.re]
  · change
      rAdd (rMulCandidate rOne z.im)
        (rMulCandidate rZero z.re) = z.im
    rw [C.mulOneLeft z.im, mul_zero_left_from_interface C z.re,
      BOMA.C.RoutePairProbe001.add_zero_right_from_interface C z.im]

theorem c_mul_one_right (C : RStageIntegrationCertificate) (z : CBOMA) :
    cMul z cOne = z := by
...
theorem c_mul_zero_left (C : RStageIntegrationCertificate) (z : CBOMA) :
...
theorem c_mul_zero_right (C : RStageIntegrationCertificate) (z : CBOMA) :
...
theorem c_ofR_neg (C : RStageIntegrationCertificate) (a : RBOMA) :
    cOfR (rNeg a) = cNeg (cOfR a) := by
  apply c_ext
  · rfl
  · exact (neg_zero_from_interface C).symm

theorem c_ofR_add (C : RStageIntegrationCertificate) (a b : RBOMA) :
    cOfR (rAdd a b) = cAdd (cOfR a) (cOfR b) := by
  apply c_ext
  · rfl
  · exact (C.addZeroLeft rZero).symm

theorem c_ofR_mul (C : RStageIntegrationCertificate) (a b : RBOMA) :
  ...
  · change
    ...
    rw [mul_zero_left_from_interface C rZero,
      neg_zero_from_interface C,
      BOMA.C.RoutePairProbe001.add_zero_right_from_interface C
        (rMulCandidate a b)]
  · change
...
theorem c_i_squared (C : RStageIntegrationCertificate) :
    cMul cI cI = cNeg cOne :=
  BOMA.C.RoutePairProbe001.imag_squared C

theorem c_generation (C : RStageIntegrationCertificate) (z : CBOMA) :
    ∃ a b : RBOMA, z = cAdd (cOfR a) (cMul (cOfR b) cI) :=
  BOMA.C.RoutePairProbe001.decomposition C z
...
theorem c_coordinate_unique
    ...
    a = a' ∧ b = b' :=
  BOMA.C.RoutePairProbe001.decomposition_unique C h

theorem c_i_not_real (C : RStageIntegrationCertificate) :
    ¬ ∃ a : RBOMA, cI = cOfR a :=
  BOMA.C.RoutePairProbe001.imag_not_real C
...
theorem cPairRingCoreCertificate
    (C : RStageIntegrationCertificate) : CPairRingCoreCertificate where
...
theorem cPairRingCoreCertificateFromAcceptedR : CPairRingCoreCertificate :=
  cPairRingCoreCertificate rStageIntegrationCertificate

end BOMA.C.PairCore001
