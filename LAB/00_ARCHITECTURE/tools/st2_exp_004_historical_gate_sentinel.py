...
PROTECTED = ("LAB/10_CONSTRUCTION/decisions/R-DP-003/UNIT.md",)
GATES: dict[str, dict[str, Any]] = {
    "A": {
        "record": "LAB/PDSA/experiments/ST2-EXP-004_GATE_A_DEPENDENCY_MAP_001.md",
        "blob": "589c4c3a65d47b7cb05fc2037f85be459c7426db",
        "run": 32835962936,
    },
    "B": {
        "record": "LAB/PDSA/experiments/ST2-EXP-004_GATE_B_NO_F04_SURVIVOR_ASSEMBLY_001.md",
        "blob": "d40daf3730e01f4aaff15cadbd24c654636a7de5",
        "run": 32835962804,
    },
    "C": {
        "record": "LAB/PDSA/experiments/ST2-EXP-004_GATE_C_CONDITIONAL_COMPARABILITY_001.md",
        "blob": "60e0fbc90aaa3edba8f416a9a03cd45a6f16f4e9",
        "run": 32835963151,
    },
    "D": {
        "record": "LAB/PDSA/experiments/ST2-EXP-004_GATE_D_SAME_CARRIER_TOTALITY_001.md",
        "blob": "5643319bbdf749da36a6bef97d9c70e287ea7af7",
        "run": 32835962907,
    },
    "E": {
        "record": "LAB/PDSA/experiments/ST2-EXP-004_GATE_E_C_SENSITIVITY_001.md",
        "blob": "63d70f4c440940abea6158a0b3ea2430796dbcc9",
        "run": 32835962908,
    },
    "F": {
        "record": "LAB/PDSA/experiments/ST2-EXP-004_GATE_F_LOGICAL_COST_001.md",
        "blob": "c83a13a0702822bc204adb50737a7bc023e6c586",
        "run": 32835963092,
    },
}
...
def main() -> int:
    ...
    for rel in PROTECTED:
        run("git", "diff", "--exit-code", BASELINE, "HEAD", "--", rel)

    record = root / gate["record"]
    if git_blob_sha(record) != gate["blob"]:
        raise SystemExit(
            f"immutable Gate-{args.gate} record drift: expected_blob={gate['blob']} actual={git_blob_sha(record)}"
        )
    ...
    if rec.get("final_gate_runs", {}).get(key) != gate["run"]:
        raise SystemExit(
            f"Gate-{args.gate} final-run drift: expected={gate['run']} actual={rec.get('final_gate_runs', {}).get(key)!r}"
        )
    ...
    result = {
        ...
        "accepted_controls_unchanged": True,
